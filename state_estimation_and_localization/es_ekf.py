...
from rotations import angle_normalize, rpy_jacobian_axis_angle, Quaternion, skew_symmetric
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    R = sensor_var * np.eye(3)  # [3, 3]

    K = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + R)  # [9, 3]
    # 3.2 Compute error state
    delta_x_k = K @ (y_k - p_check)
    # delta_x_k = K * (y_k - p_check[:3])  # check which dimensions are needed to get only position error
    # 3.3 Correct predicted state

    delta_p = delta_x_k[0:3]  # [3]
    delta_v = delta_x_k[3:6]  # [3]
    delta_phi = delta_x_k[6:9]  # [3]
    delta_phi = angle_normalize(delta_phi)  # [3]

    p_hat = p_check + delta_p
    v_hat = v_check + delta_v
    q_hat = Quaternion(axis_angle=delta_phi).quat_mult_left(q_check)

    # 3.4 Compute corrected covariance

    p_cov_hat = (np.eye(9) - K @ h_jac) @ p_cov_check  # [9, 9]
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    Cns = Quaternion(*q_est[k - 1])
    Cns_mat = Cns.to_mat()
    # 1.1 Linearize the motion model and compute Jacobians

    fns_g = Cns_mat @ imu_f.data[k - 1]
    fns = fns_g + g
    # imu = imu_f.data[k - 1]
    p_check = p_est[k - 1] + delta_t * v_est[k - 1] + (delta_t ** 2 / 2) * fns

    v_check = v_est[k - 1] + delta_t * fns

    q = Quaternion(euler=imu_w.data[k - 1] * delta_t).normalize().quat_mult_right(Cns, out='np')
    # 2. Propagate uncertainty
    F_last = np.eye(9)
    F_last[:3, 3:6] = np.eye(3) * delta_t
    F_last[3:6, -3:] = - (skew_symmetric(fns_g)) * delta_t

    Q_last = Q * (delta_t ** 2)
    p_cov_check = F_last.dot(p_cov[k - 1]).dot(F_last.T) + l_jac.dot(Q_last).dot(l_jac.T)

    # 3. Check availability of GNSS and LIDAR measurements

    # lidar
    if len(np.where(lidar.t == t)[0]) == 1:
        idx = np.where(lidar.t == t)[0][0]
        p_check, v_check, q, p_cov_check = measurement_update(var_lidar, p_cov_check, lidar.data[idx],
                                                              p_check, v_check, q)
        lidar_i += 1
    # gnss
    if len(np.where(gnss.t == t)[0]) == 1:
        idx = np.where(gnss.t == t)[0][0]
        p_check, v_check, q, p_cov_check = measurement_update(var_gnss, p_cov_check, gnss.data[idx],
                                                              p_check, v_check, q)
        gnss_i += 1
    # Update states (save)
