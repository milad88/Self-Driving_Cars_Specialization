    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                # circle_locations[:, 0] = [path[0][j] + i * cos(path[2]) for i in self._circle_offsets]
                # circle_locations[:, 1] = [path[1][j] + i * cos(path[2]) for i in self._circle_offsets]
                circle_locations[:, 0] = [i * np.cos(path[2][j]) for i in self._circle_offsets] + path[0][j]
                circle_locations[:, 1] = [i * np.sin(path[2][j]) for i in self._circle_offsets] + path[1][j]
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = np.sqrt(np.power((goal_state[0] - paths[i][0][-1]), 2) +
                                np.power((goal_state[1] - paths[i][1][-1]), 2))
                # --------------------------------------------------------------
